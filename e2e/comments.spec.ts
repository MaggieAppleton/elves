...
test('a freeform comment can be resolved away', async ({ page, request }) => {
  ...
  const resolveFirst = page.getByRole('button', { name: 'Resolve comment 1 of 2: freeform note' })
  const resolveSecond = page.getByRole('button', { name: 'Resolve comment 2 of 2: second note' })
  await expect(resolveFirst).toHaveCount(1)
  await expect(resolveSecond).toHaveCount(1)
  await resolveFirst.click()
  await expect(comment).toHaveCount(1)
  await page.getByRole('button', { name: 'Resolve comment 1 of 1: second note' }).click()
  await expect(page.locator('.elves-comment')).toHaveCount(0)
})
