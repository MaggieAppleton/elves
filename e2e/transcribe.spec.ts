...
test('create_note_card renders as a remote change outside local undo history', async ({ page, request }) => {
  await page.goto('/')
  await expect(page.locator('.tl-canvas')).toBeVisible({ timeout: 15000 })
  await expect(page.getByTestId('new-prose')).toBeEnabled()
  await page.waitForTimeout(200) // let the realtime socket connect before posting
  ...
  await expect(page.locator('.elves-card--note', { hasText: 'my handwriting, typed' })).toHaveCount(1)
})
